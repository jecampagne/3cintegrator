...
/*!
  \brief Class common two symmetric and non-symmetric product
 */

class ChebyshevIntBase {
 public:
  ChebyshevIntBase(): 
    nOrdProd_(0), planInv_(0), planCC_(0) {
  }

  virtual ~ChebyshevIntBase() {
...
//     }
    ...
    std::vector<r_8> kin(val.size());
    //JEC 17/6/17 put k_i in increasing order
    for(k=0;k<=n;k++){
      //      kin[k] = cos(k*cte)*bma+bpa;
...
/*! Symmetric version: to perform Int f(x; para1) * f(x; para2) dx  JEC 14/2/17
  use case: C_ell(z1, z2) computation
 */

///////////////////////////////////////////

class ChebyshevIntSym : public ChebyshevIntBase {
 public:
  ChebyshevIntSym(int ordFunc): 
    ChebyshevIntBase(),
    ordFunc_(ordFunc),
    planFunc_(0)
    {
  ...
  void Init();
    ...
    */
  void ChebyshevTransform(ClassFunc1D* func, std::vector<r_8>& vec, r_8 lowBnd, r_8 uppBnd){

    ChebyshevSampling(vecDCTFunc_,*func,lowBnd,uppBnd);
    ChebyshevCoeffFFT(*planFunc_, vecDCTFunc_);
    std::fill(vecDCTInv_.begin(), vecDCTInv_.end(), 0.0);
    std::copy(vecDCTFunc_.begin(),vecDCTFunc_.end(),vecDCTInv_.begin());
    InverseChebyshevCoeffFFT();
    vec.resize(vecDCTInv_.size());
    std::copy(vecDCTInv_.begin(), vecDCTInv_.end(), vec.begin());
 ...
 private:   
  int ordFunc_; //!< order of Chebychev polynomial
  int nOrdFunc_; //!< size of the vector vecDCTFunc_

  std::vector<r_8> vecDCTFunc_; //!< vector used by FFTW for func
  FFTPlanning*  planFunc_; //!< FFTW plan for func, it uses  vecDCTFunc_
...
/*! Non Symmetric version: to perform Int f_1(x; para1) * f_2(x; para2) dx  JEC 14/2/17

 */

///////////////////////////////////////////

class ChebyshevIntNonSym : public ChebyshevIntBase {
 public:
  ChebyshevIntNonSym(int ordFunc1, int ordFunc2): 
    ChebyshevIntBase(),
    ordFunc1_(ordFunc1), ordFunc2_(ordFunc2),
    planFunc1_(0), planFunc2_(0)
    {
      Init();
    }
  virtual ~ChebyshevIntNonSym() {
  ...
  void ChebyshevTransform(ClassFunc1D* f1, ClassFunc1D* f2, std::vector<r_8>& vec, r_8 lowBnd, r_8 uppBnd){
     ...
     if(f1){
       ...
       std::fill(vecDCTInv_.begin(), vecDCTInv_.end(), 0.0);
       std::copy(vecDCTFunc1_.begin(),vecDCTFunc1_.end(),vecDCTInv_.begin());
       InverseChebyshevCoeffFFT();
       vec.resize(vecDCTInv_.size());
       std::copy(vecDCTInv_.begin(), vecDCTInv_.end(), vec.begin());
       ...
       std::fill(vecDCTInv_.begin(), vecDCTInv_.end(), 0.0);
       std::copy(vecDCTFunc2_.begin(),vecDCTFunc2_.end(),vecDCTInv_.begin());
       InverseChebyshevCoeffFFT();
       vec.resize(vecDCTInv_.size());
       std::copy(vecDCTInv_.begin(), vecDCTInv_.end(), vec.begin());
